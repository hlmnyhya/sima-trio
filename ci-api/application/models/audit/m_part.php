...
class M_Part extends CI_Model {
    ...
    { 
        $query = "
        INSERT INTO part (part_number, kd_lokasi_rak, id_cabang, id_lokasi, deskripsi, qty, harga_jual, kondisi, status, tanggal_audit, idjadwal_audit)
        SELECT part_number, kd_lokasi_rak, id_cabang, id_lokasi, deskripsi, qty, harga_jual, kondisi, status, CONVERT(date,GETDATE()) as tanggal_audit, idjadwal_audit
        FROM temp_part a 
    ...
    public function previewpart($cabang, $idjadwal_audit, $offset)
    {
        $this->db->select('
        ...
        ');
            ...
            $this->db->where("a.id_cabang='$cabang' AND a.idjadwal_audit = '$idjadwal_audit' ");
            $this->db->limit(15, $offset);
            $this->db->offset($offset);
