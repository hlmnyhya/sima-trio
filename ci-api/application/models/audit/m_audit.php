...
public function GetListAudPart($id = null, $cabang = null, $idjadwal_audit = null)
    ...
    {
        if ($id === null) {
            $this->db->select('a.id_part,a.id_cabang, a.id_lokasi,a.part_number,a.deskripsi,a.qty, a.qty_fsk, a.selisih,a.harga_jual,a.kd_lokasi_rak,a.status,a.kondisi,a.keterangan,b.nama_cabang, c.nama_gudang');
            $this->db->from('part a');
            $this->db->join('cabang b', 'a.id_cabang = b.id_cabang', 'left');
            $this->db->join('gudang c', 'a.id_lokasi = c.kd_gudang', 'left');
            $this->db->where('a.id_cabang', $cabang);
