...
class Master_Data extends CI_Controller
{
    ...
    public function gudang()
    {
        ...
        $this->load->view('general_affairview/gudang/_partial/footerv_gudang.php');
    }
            ...
            if ($this->session->userdata('usergroup') == 'UG005') {
                foreach ($listUser as $list) {
                    ...
                    if ($list['status'] == 'Aktif') {
                        ...
                        $hapus = "<a href='" . $base . "master_data/delete_user/" . $list['nik'] . "' class='text-danger' onclick=\"return confirm('Konfirmasi menghapus data " . $list['nik'] . "');\"><i class='fa fa-trash'></i></a>";
                    }
                    $output .= "
                            <tr>
        ...
        foreach ($listUserGroup as $list) {
            ...
            $output .= '
                <tr> 
                    ...
                    <td class="">' . $list['id_usergroup'] . '</td>
                    <td class="">' . $list['user_group'] . '</td>
                </tr>
    ...
    public function ajax_get_perusahaan()
    {
        ...
        $config['base_url'] = base_url() . 'laporan_auditor/ajax_get_perusahaan';
        $config['total_rows'] = $count;
        $config['per_page'] = 15;
        $config['uri_segment'] = 3;
        $config['use_page_numbers'] = TRUE;
    ...
    public function ajax_get_gudang()
    {
        ...
        foreach ($listLokasi as $list) {
            ...
            <tr> 
                <td class="text-center">' . $start . '</td>
                <td class="text-center">
                <a onclick="edit(id=\'' . $list['kd_gudang'] . '\')" class="text-warning" ><i class="fa fa-pencil"></i></a>
                <a href="' . $base . 'master_data/delete_gudang/' . $list['kd_gudang'] . '" class="text-danger" onclick=\'return confirm("Konfirmasi menghapus data ' . $list['kd_gudang'] . ' - ' . $list['nama_gudang'] . ' ? ");\'><i class="fa fa-trash"></i></a>
                </td>
                <td>' . $list['kd_gudang'] . '</td>
                <td>' . $list['nama_gudang'] . '</td>
                <td>' . $list['jenis_gudang'] . '</td>
                <td>' . $list['alamat'] . '</td>
            </tr>
    ...
    public function input_gudang()
    {
        ...
        $this->load->view('general_affairview/gudang/_partial/footerinput_gudang.php');
    }
    ...
    public function post_Gudang()
    {
        $data = [
            ...
            'created_by'  => html_escape($this->session->userdata('username'))
        ];
    ...
    public function search_data_subinventory()
    {
        ...
        $config['base_url'] = base_url() . 'master_data/search_data_subinventory';
        $config['total_rows'] = $count;
        $config['per_page'] = 15;
        $config['uri_segment'] = 3;
        $config['use_page_numbers'] = TRUE;
