...
<script>
    $(document).ready(function() {
        ...
        function rule() {
            var id = $('#Optusergroup').val();
            $('#rule').html('<tr><td colspan="13" id="loading"></td></tr>')
            $.ajax({
                type: 'POST',
                url: "<?php echo base_url(); ?>menu/ajax_get_rule",
                dataType: 'JSON',
                data: {
                    id: id
                },
                success: function(data) {
                    $('#rule').html(data.output);
                }
            })
        }
    });
    ...
    function show() {
        ...
        $.ajax({
            url: "<?php echo base_url() . $this->uri->segment(1) ?>/input_rules",
            type: 'post',
            dataType: 'html',
            success: function(data) {
