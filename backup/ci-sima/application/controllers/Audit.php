...
class Audit extends CI_Controller
{
    public function __construct()
    {
        ...
        if (!$this->session->userdata('username')) {
            $this->session->set_flashdata(
                'pesan',
                '<div class="alert alert-danger">Silakan Login lebih dulu!</div>'
            );
    ...
    public function ajax_get_jadwal_audit()
    {
        ...
        if ($listJadwalAudit) {
                ...
                if ($list['keterangan'] == 'waiting') {
                    ...
                    if ($list['jenis_audit'] == 'Audit Unit') {
                        ...
                        $link =
                            ...
                            $list['id_cabang'] .
                            '&&a=' .
                            base64_encode($list['idjadwal_audit']);
                        $list['keterangan'] =
                            '
                        <a class="btn btn-success" onClick="MyWindow=window.open(\'' .
                            $link .
                            '\',\'MyWindow\',\'width=683,height=576\'); return false;">BUKA</a>
                        ';
                    } elseif ($list['jenis_audit'] == 'Audit Part') {
                        $link =
                            base_url() .
                            'transaksi/auditPart?id=' .
                            $list['id_cabang'] .
                            '&&a=' .
                            base64_encode($list['idjadwal_audit']);
                        $list['keterangan'] =
            ...
            $output =
                '<tr>
                <td colspan = "9" class="text-center">Belum ada data.<br /><a href="' .
                base_url() .
    ...
    public function edit_Jadwal_Audit()
    {
        ...
        $this->load->view('_partial/sidebar.php');
        $this->load->view(
            'auditorview/jadwal_audit/v_edit_jadwal_audit.php',
            $data
        );
        $this->load->view('auditorview/jadwal_audit/_partial/footer2.php');
    }
    ...
    public function search_data_jadwal_audit()
    {
        ...
        if (
            ...
            $listJdwAudit = $this->maudit->carijadwalaudit(
                $auditor,
                $tanggal_audit,
                $jenis_audit
            );
